...
const PRIMARY_KEY = 'themePrimary';
const MENU_MODE_KEY = 'menuMode';

// ── Asosiy rang (primary color) ──────────────────────────────────────────
// AppConfigurator.vue (topbardagi tez sozlamalar) va Profile.vue ("Ko'rinish" tabi)
// bir xil ro'yxat va o'rnatish mantig'idan foydalanadi — dublikat qilinmasin.
...
export function getPrimaryPresetExt(colorName) {
    const color = PRIMARY_COLORS.find((c) => c.name === colorName) || PRIMARY_COLORS[0];
...
export function setPrimaryColor(colorName) {
    layoutConfig.primary = colorName;
    localStorage.setItem(PRIMARY_KEY, colorName);
    updatePreset(getPrimaryPresetExt(colorName));
}
...
const layoutConfig = reactive({
    preset: 'Aura',
    primary: localStorage.getItem(PRIMARY_KEY) || 'emerald',
    surface: null,
    darkTheme: false,
    // Mavzu rejimi: 'light' | 'dark' | 'system' (default — tizim, ya'ni OS sozlamasiga ergashadi)
    themeMode: localStorage.getItem(THEME_KEY) || 'system',
    menuMode: localStorage.getItem(MENU_MODE_KEY) || 'static'
});

// Sahifa yuklanganda saqlangan asosiy rang standart (emerald)dan farq qilsa,
// PrimeVue preset'iga qayta qo'llaymiz — aks holda faqat layoutConfig.primary
// yangilanadi-yu, haqiqiy ranglar sahifa yangilanguncha eski (emerald) bo'lib qoladi.
if (typeof window !== 'undefined' && layoutConfig.primary !== 'emerald') {
    updatePreset(getPrimaryPresetExt(layoutConfig.primary));
}
...
// Rejimga qarab qorong'i mavzu yoqilishi kerakmi?
function resolveDark(mode) {
    if (mode === 'dark') return true;
    if (mode === 'light') return false;
    return prefersDark ? prefersDark.matches : false; // 'system'
}

// Joriy rejimni <html>.app-dark klassiga qo'llaydi.
function applyTheme() {
    const dark = resolveDark(layoutConfig.themeMode);
    layoutConfig.darkTheme = dark;
    document.documentElement.classList.toggle('app-dark', dark);
}

function setThemeMode(mode) {
    layoutConfig.themeMode = mode;
    localStorage.setItem(THEME_KEY, mode);
    applyTheme();
}

// Dastlabki qo'llash + 'system' rejimida OS mavzusi o'zgarishini kuzatish.
if (typeof window !== 'undefined') {
    applyTheme();
    prefersDark?.addEventListener?.('change', () => {
        if (layoutConfig.themeMode === 'system') applyTheme();
    });
}
...
const layoutState = reactive({
    ...
    activeMenuItem: null,
    sidebarPanel: 'module' // 'module' | 'hududlar'
});
...
export function useLayout() {
    ...
    const toggleDarkMode = () => setThemeMode(layoutConfig.darkTheme ? 'light' : 'dark');

    const themeMode = computed(() => layoutConfig.themeMode);
